...
let colorArray = [];
    ...
    const api = "1RIBwUysK44DNjo_fuI6kASXDAuPqeYdzpdVqHX9zUnQ";
    let piechartList = document.querySelector("#piechartList");

    document.addEventListener("DOMContentLoaded", getJSON);
    async function getJSON() {
        let result = await fetch(`https://spreadsheets.google.com/feeds/list/${api}/5/public/values?alt=json`);
    ...
    function doThePie(){
        theJson.feed.entry.forEach((each, index) =>{
            colorArray.push(each.gsx$color.$t);
            ...
            piePiece.style.stroke = colorArray[index];
            
            const newLi = document.createElement("li");

            newLi.style.backgroundColor = colorArray[index];
